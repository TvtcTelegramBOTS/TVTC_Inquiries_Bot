...
if not BOT_TOKEN:
    print("❌ لم يتم العثور على متغير TELEGRAM_TOKEN. تأكد من إضافته في إعدادات Render.")
    sys.exit(1)

# =========================
# حالة البوت (تُعرض للداشبورد)
# =========================
STATUS = {
    "running": False,
...
def compress_pdf_with_ghostscript(input_file: str, output_file: str, max_size_mb: float = 3.0):
    ...
    try:
        ...
        print("✅ تم ضغط الملف بنجاح", flush=True)
        if os.path.getsize(output_file) / (1024*1024) > max_size_mb:
            print("⚠️ الحجم لا يزال كبيراً، سيتم استخدام النسخة الأصلية.", flush=True)
            os.remove(output_file)
            os.rename(input_file, output_file)
    except subprocess.CalledProcessError:
        print("❌ فشل الضغط، سيتم استخدام النسخة الأصلية.", flush=True)
        os.rename(input_file, output_file)
    except Exception as e:
        print(f"⚠️ تعذر الضغط ({e})، سيتم استخدام النسخة الأصلية.", flush=True)
        try:
            os.rename(input_file, output_file)
        except Exception:
            pass
...
async def send_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE, service: str):
    ...
    try:
        ...
        if service == "remaining":
            ...
            idx = sorted_students.index((student_id, start))
            end = sorted_students[idx+1][1] if idx < len(sorted_students)-1 else len(reader.pages)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "📤 تسجيل الخروج":
        ...
        # زر إعادة الدخول
        keyboard = [[KeyboardButton("🔁 إعادة تسجيل الدخول")]]
        await update.message.reply_text(
